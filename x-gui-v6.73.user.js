...
// @downloadURL https://update.greasyfork.org/scripts/570602/X-GUI%20CLIENT%20FOR%20BLOOKET.user.js
// @updateURL https://update.greasyfork.org/scripts/570602/X-GUI%20CLIENT%20FOR%20BLOOKET.meta.js
// ==/UserScript==

var window = unsafeWindow; // Super ultra op fix by the one and only landsedge
...
console.log(
  "%c X-GUI CLIENT LOADED! ENJOY! %c\n Created by Xullys on GitHub",
  "color: #0bc2cf; font-size: 3rem",
  "color: #8000ff; font-size: 1rem",
);
console.log("%c gui.js", "color: #0bc2cf; font-size: 1rem");
console.log(
  "%c Star the GitHub repo! %c https://github.com/xgui-client/X-GUI-BLOOKET-CLIENT",
...
(() => {
  ...
  const cheat = async () => {
    ...
    try {
      ...
      for (const variable in Settings.data.theme || {})
        gui.style.setProperty("--" + variable, Settings.data.theme[variable]);
...
.creditsPage>ul>li>span{color:var(--textColor2);font-weight:800}.creditsPage>ul>li i{margin-inline:2px;line-height:1}.creditsPage a{color:var(--highlight);text-decoration:none}.creditsPage a:hover,.pathPage:hover{text-decoration:underline}.warning{color:var(--highlight2);font-size:.85em}.searchResult{cursor:pointer}.searchResultName{font-weight:800}.searchResultDescription{font-size:.8em}.searchResultSeparator{font-size:1.5em;font-weight:800;margin-block:10px;cursor:pointer;transition:.2s;border-bottom:2px solid #fff;padding-inline:5px;filter:drop-shadow(0px 0px 0px var(--highlight))}.searchResultSeparator:hover{color:var(--highlight);border-bottom:2px solid var(--highlight);filter:drop-shadow(0px 0px 2.5px var(--highlight))}.toggleCheat{--buttonColor:var(--toggleOff)}.toggleCheat.active{--buttonColor:var(--toggleOn)}.logMessage img{height:1em;align-self:center}.standing{display:flex;font-weight:800;align-items:center;position:relative;padding:5px 10px 5px 50px;border-radius:2.5px;background:var(--highlight2)}.standing:before{content:attr(data-place) ".";margin-right:10px}.standing::after{content:attr(data-value);flex:1;text-align:right;font-weight:100}.standing:hover{background:var(--standingColor);box-shadow:0 0 7.5px var(--standingColor)}.standingBlook{height:1.25em;align-self:center;position:absolute;left:10px}.favoriteButton,.favoriteButton>i{transition:.2s;display:grid;place-items:center;width:32px;height:32px}.favoriteButton{font-size:.8em;padding-left:5px;cursor:pointer}.favoriteButton:hover{color:#ff0}.favoriteButton>i{position:absolute;scale:0;transform-origin:50% 55%}.favoriteButton>i.filled{scale:1}.pathPage{cursor:pointer;color:var(--highlight)}[data-favorited=false],[data-favorites="0"]{display:none}
        ...
        .replace(
          /\.([^0-9][\w-]+)/gm,
          (x, y) => "." + (classes[y] ??= randString(10)),
        )
        .replace(
          /data-(\w+)/gm,
          (x, y) => "data-" + (datasets[y] ??= randString(10)),
        );
...
`;
    ...
    function setMinIcon(min) {
      minimizeControl.innerHTML = `<i class="xgui-i ${min ? "xgui-min" : "xgui-full"}"></i>`;
    }
    setMinIcon(true);
    ...
    function saveGUIState() {
      try {
        ...
        document.cookie =
          ...
          "; path=/; domain=.blooket.com; expires=Tue, 19 Jan 2038 03:14:07 GMT";
      } catch {}
    }
    ...
    leaderboardList.className =
      classes.noScroll + " " + classes.leaderboardList;
    ...
    const Logs = {
      ...
      exponents: ["⁰", "¹", "²", "³", "⁴", "⁵", "⁶", "⁷", "⁸", "⁹"],
      formatNumber(input) {
        const [number, exponent] = (input = parseFloat(input))
          ...
          .split("e");
        if (exponent < 15)
          return number + ["", "k", "M", "B", "T"][exponent / 3];
        const [num, exp] = input
          .toLocaleString(undefined, {
            notation: "scientific",
          })
        ...
        return (
          num +
          " \xd7 10" +
          exp.split("").reduce((a, b) => a + Logs.exponents[b], "")
        );
      },
      leaderboardCache: {},
      ...
      setLeaderboard(standings) {
        ...
        for (let i = 0; i < standings.length; i++) {
          ...
          const standingEl =
            Logs.leaderboardCache[standing.name] ||
            Logs.createStandingElement(standing.name);
          standingEl.firstChild.src = (blookInfo =
            Logs.blookData[Logs.data[standing.name]?.b || "Black"]).url;
          standingEl.style.setProperty("--standingColor", blookInfo.color);
          standingEl.dataset[datasets.value] = Logs.formatNumber(
            standing.value,
          );
      ...
      fetchBlooks() {
        return (
          ...
          new Promise((r) => {
            ...
            s.type = "module";
            s.src =
              document.querySelector("script[src*='ac.blooket.com']").src +
              "?" +
              Date.now();
            ...
            let finish;
            i.contentWindow.Object.prototype.hasOwnProperty.call = function (
              a,
              b,
            ) {
              if (a[b]?.rarity && a in blooks == false)
                Object.assign(blooks, a);
              finish ??= setTimeout(() => {
      ...
      async connect() {
        try {
          ...
          Logs.connection.on("value", (snapshot) => {
            ...
            let added;
            if (!players || !(added = Logs.diffObjects(Logs.data, players)))
              return;
            Logs.data = players;
              ...
              switch (gamemode) {
                case "brawl":
                  for (const player in added) {
                    ...
                    if (
                      upgrade.length == 2 &&
                      upgrade[0] in Logs.gamemodeData.brawl.upgrades
                    )
                      Logs.addAlert(
                        player,
                        `upgraded ${Logs.gamemodeData.brawl.upgrades[upgrade[0]]} to level ${upgrade[1]}`,
                      );
                  }
                  break;
                case "gold":
                  for (const player in added) {
                    ...
                    if (amount == "swap")
                      Logs.addAlert(
                        player,
                        `just swapped ${document.querySelector("[src*='assets/candy']") ? "candy" : "gold"} with ${tat}`,
                      );
                    else
                      Logs.addAlert(
                        player,
                        `just took ${Logs.formatNumber(parseInt(amount))} ${document.querySelector("[src*='assets/candy']") ? "candy" : "gold"} from ${tat}`,
                      );
                  }
                  break;
                case "toy":
                  for (const player in added) {
                    if ((addedPlayer = added[player]).s)
                      Logs.addAlert(
                        player,
                        `sabotaged with "${Logs.gamemodeData.toy.sabotages[addedPlayer.s]}"`,
                      );
                    else if (addedPlayer.tat) {
                      const [tat, amount] = addedPlayer.tat.split(":");
                      if (amount == "swap")
                        Logs.addAlert(player, `just swapped toys with ${tat}`);
                      else
                        Logs.addAlert(
                          player,
                          `just took ${Logs.formatNumber(parseInt(amount))} toy${amount == 1 ? "" : "s"} from ${tat}`,
                        );
                    }
                  }
                  break;
                case "hack":
                  for (const player in added) {
                    ...
                    Logs.addAlert(
                      player,
                      `just took ${Logs.formatNumber(parseInt(amount))} crypto from ${tat}`,
                    );
                  }
                  break;
                case "pirate":
                  for (const player in added) {
                    ...
                    Logs.addAlert(
                      player,
                      `just took ${Logs.formatNumber(parseInt(amount))} doubloons from ${tat}`,
                    );
                  }
                  break;
                case "defense2":
                  for (const player in added) {
                    ...
                    Logs.addAlert(
                      player,
                      `just completed Round ${addedPlayer.r}!`,
                    );
                  }
                  break;
                case "fishing":
                  for (const player in added) {
                    if ((addedPlayer = added[player]).f == "Frenzy")
                      Logs.addAlert(player, `just started a frenzy`);
                    else if (addedPlayer.s)
                      Logs.addAlert(
                        player,
                        `just sent a ${addedPlayer.f} distraction`,
                      );
                  }
                  break;
                case "dino":
                  for (const player in added) {
                    ...
                    if (caught == "true")
                      Logs.addAlert(player, `just caught ${tat} CHEATING!`);
                ...
                case "factory":
                  for (const player in added) {
                    if ((addedPlayer = added[player]).g)
                      Logs.addAlert(
                        player,
                        `activated the ${Logs.gamemodeData.factory.glitches[addedPlayer.g]} glitch!`,
                      );
                    else if (addedPlayer.s) {
                    ...
                    } else if (addedPlayer.t)
                      Logs.addAlert(player, `now has 10 Blooks!`);
                  }
                  break;
              }
            } catch (e) {
      ...
      addLog(message, color) {
        ...
        span.innerHTML =
          "[LOG] " +
          Logs.sanitizeText(message) +
          `<span style="opacity: 50%; flex: 1; text-align: right;">${parseTime(Logs.lastLog)}</span>`;
        element.append(span);
        logMessages.prepend(element);
      },
      interval: null,
    ...
    if (
      window.location.host != "dashboard.blooket.com" &&
      window.location.host != "blooket.com"
    )
      Logs.interval = setInterval(
        () =>
          Logs.connect().then(
            (connected) => connected && clearInterval(Logs.interval),
          ),
        5000,
      );
      ...
      global: {
        ...
        cheats: [
            ...
            run: function () {
              if (!this.enabled) {
                ...
                this.data = setInterval(() => {
                  const stateNode = getStateNode();
                  const Question =
                    stateNode.state.question || stateNode.props.client.question;
                  if (stateNode.state.question.qType != "typing") {
                    if (
                      stateNode.state.stage != "feedback" &&
                      ...
                      for (ind = 0; ind < Question.answers.length; ind++) {
                        ...
                        for (let j = 0; j < Question.correctAnswers.length; j++)
                          if (
                            Question.answers[ind] == Question.correctAnswers[j]
                          ) {
                            found = true;
                            break;
                          }
                      ...
                      document
                        .querySelector("[class*='feedback'], [id*='feedback']")
                        .firstChild.click();
                  } else
                    Object.values(
                ...
                this.data = setInterval(() => {
                  ...
                  while (ind < Question.answers.length) {
                    ...
                    document.querySelector(
                      "[class*='answersHolder'] :nth-child(" + ind + ") > div",
          ...
          },
          {
            ...
            description:
              "Toggles Learn Auto Answer on. This version works in the modes that auto answer doesnt work in, but can only learn answers if you've seen them before.",
            type: "toggle",
            enabled: false,
            data: null,
            run: function () {
                ...
                (() => {
                  ...
                  function learn() {
                    ...
                    wrappers.forEach((wrapper) => {
                      const questionTextEl = wrapper.querySelector(
                        '[class*="questionText"]',
                      );
                      if (!questionTextEl) return;
                      let questionMedia = wrapper.querySelector(
                        '[class*="questionImage"]',
                      );
                      let qMediaSrc = questionMedia
                        ? questionMedia.src
                        : "none";
                      let questionMath = wrapper.querySelector(
                        '[class*="mq-selectable"]',
                      );
                      let qMathText = questionMath
                        ? questionMath.innerText.trim()
                        : "none";
                      const question =
                      ...
                      const correctBtn = Array.from(
                        wrapper.querySelectorAll('[class*="answerButton"]'),
                      ).find(
                        (btn) =>
                          btn.innerHTML.includes("fa-check") ||
                          (btn.style.backgroundColor &&
                            btn.style.backgroundColor.includes(
                              "rgb(139, 220, 111)",
                            )) ||
                          (btn.querySelector('[class*="answerFront"]') &&
                            btn
                              .querySelector('[class*="answerFront"]')
                              .style.backgroundColor.includes(
                                "rgb(139, 220, 111)",
                              )),
                      );
                      if (correctBtn) {
                        const textEl = correctBtn.querySelector(
                          '[class*="answerText"]',
                        );
                        const mathEl = correctBtn.querySelector(
                          '[class*="mq-selectable"]',
                        );
                        const answer = textEl
                        ...
                        if (answer && !window.answerDatabase[question]) {
                          ...
                          console.log(
                            `✓ Learned (MC): "${question}" → "${answer}"`,
                          );
                        }
                      }
                      const rawTypingEls = Array.from(
                        wrapper.querySelectorAll(
                          '[class*="typingFeedbackAnswer"]',
                        ),
                      );
                      const realTypingAnswers = rawTypingEls.filter((el) => {
                        const cls = el.getAttribute("class") || "";
                        return (
                          !cls.includes("Answers") && !cls.includes("Icon")
                        );
                      });
                      if (realTypingAnswers.length > 0) {
                        ...
                        if (!window.answerDatabase[question]) {
                          window.answerDatabase[question] = [answer];
                          console.log(
                            `✓ Learned (Typing): "${question}" → "${answer}"`,
                          );
                        }
                      }
                    });
                  ...
                  function answer() {
                    ...
                    const questionTextEl = activeWrapper.querySelector(
                      '[class*="questionText"]',
                    );
                    if (!questionTextEl) return;
                    let questionMedia = activeWrapper.querySelector(
                      '[class*="questionImage"]',
                    );
                    let qMediaSrc = questionMedia ? questionMedia.src : "none";
                    let questionMath = activeWrapper.querySelector(
                      '[class*="mq-selectable"]',
                    );
                    let qMathText = questionMath
                      ? questionMath.innerText.trim()
                    ...
                    if (window.answerDatabase[question]) {
                      ...
                      const buttons = activeWrapper.querySelectorAll(
                        '[class*="answerButton"]',
                      );
                      for (const btn of buttons) {
                        ...
                        const mathEl = btn.querySelector(
                          '[class*="mq-selectable"]',
                        );
                        const btnText = textEl
                          ? textEl.textContent.trim()
                          : mathEl
                            ? mathEl.innerText.trim()
                            : "";
                      ...
                      const input = activeWrapper.querySelector(
                        'input[class*="typingAnswerInput"]',
                      );
                      if (input && !input.disabled) {
                        ...
                        const submitBtn = activeWrapper.querySelector(
                          'button[class*="typingAnswerButton"]',
                        );
                        if (
                          submitBtn &&
                          (!submitBtn._lastClick ||
                            Date.now() - submitBtn._lastClick > 200)
                        ) {
                          submitBtn.click();
          ...
          },
          {
            ...
            description:
              "Toggles Learn Highlight Answer on. This version works in the modes that highlight answers doesnt work in, but can only learn answers if you've seen them before.",
            type: "toggle",
            enabled: false,
            data: null,
            run: function () {
              if (!this.enabled) {
                ...
                (() => {
                  console.log(
                    "🎨 Highlight Answers Active (Green = Correct, Red = Wrong)\n",
                  );
                  if (!window.answerDatabase) {
                    window.answerDatabase = {};
                  }
                  ...
                  function highlight() {
                    ...
                    if (window.answerDatabase[question]) {
                        ...
                        buttons.forEach((btn) => {
                          ...
                          const btnText = textEl
                            ...
                            : mathEl
                              ? mathEl.innerText.trim()
                              : "";
                          const front = btn.querySelector(
                            '[class*="answerFront"]',
                          );
                          const back = btn.querySelector(
                            '[class*="answerBack"]',
                          );
                      ...
                      if (input) {
                        ...
                        const subHeader = activeWrapper.querySelector(
                          '[class*="typingAnswerSubheader"]',
                        );
                        if (subHeader) {
                  ...
                  console.log(
                    "✅ Highlighting Active. Play manually to populate database.",
                  );
                })();
              } else {
                this.enabled = false;
          ...
          },
          {
            ...
            run: function () {
              if (!this.enabled) {
                ...
                (() => {
                  console.log(
                    "🕵️ Subtle Highlight Active (Lighter Shadow = Correct)\n",
                  );
                  if (!window.answerDatabase) {
                    window.answerDatabase = {};
                  }
                  function lightenColor(color) {
                    ...
                    if (!rgb) return color;
                    const r = Math.round(
                      parseInt(rgb[0]) + (255 - parseInt(rgb[0])) * 0.1,
                    );
                    const g = Math.round(
                      parseInt(rgb[1]) + (255 - parseInt(rgb[1])) * 0.1,
                    );
                    const b = Math.round(
                      parseInt(rgb[2]) + (255 - parseInt(rgb[2])) * 0.1,
                    );
                    return `rgb(${r}, ${g}, ${b})`;
                  }
                  function autoContinue() {
                      ...
                      if (buttons.length > 0) {
                        buttons.forEach((btn) => {
                          ...
                          if (back && btnText === knownAnswer) {
                            const currentColor =
                              window.getComputedStyle(back).backgroundColor;
            ...
            run: function () {
              ...
              if (this.enabled)
                ((this.enabled = !1),
                  clearInterval(this.data),
                  (this.data = null),
                  e.props.liveGameController.removeVal(
          ...
          },
          {
            name: "Anti Kick",
            description:
              "Prevents the host from kicking you, Although cheats stop working and you can't win.",
            type: "toggle",
            enabled: false,
            data: null,
            run: function () {
                  ...
                  (TARGET) => {
                    try {
                      ...
                      if (
                        ...
                        let correct = 0,
                          total = 0;
                        for (let corrects in stateNode.corrects)
                          correct += stateNode.corrects[corrects];
                        for (let incorrect in stateNode.incorrects)
                          total += stateNode.incorrects[incorrect];
                        total += correct;
                        const yes =
                          total == 0 ||
                          Math.abs(correct / (total + 1) - TARGET) >=
                        ...
                        if (stateNode.state.question.qType != "typing") {
                          ...
                          for (let i = 0; i < answerContainers.length; i++) {
                            const contains = question.correctAnswers.includes(
                              question.answers[i],
                            );
                            if (yes == contains)
                              return answerContainers[i]?.click?.();
                          }
          ...
          },
          {
            name: "Free Folder!",
            description:
              "Gives you plus perks like folders, does not unlock plus gamemodes! (ONLY WORKS ON THE MY SETS PAGE!)",
            run: function () {
              const reactHandler = () =>
              ...
              if (!component)
                return console.error("React component not found!");
            ...
            inputs: [
              {
                ...
                options: () =>
                  [
                    ...document.querySelector("[class*=packsWrapper]")
                      ?.children,
                  ]?.map((e) => e.children[0].children[0].alt),
              },
            ],
            run: async (packName) => {
              ...
              function pickW(a) {
                ...
                const rand = Math.floor(
                  Math.random() * a.reduce((a, b) => a + b, 0),
                );
                a.forEach((e) => (sum.push(v), (v += e)));
                return sum
                  .map((e) => rand < e)
                  .findLastIndex((e) => (e ? 0 : 1));
              }
              stateNode.setState({
                loadingPack: !1,
                openPack: !0,
                ...
                options: () =>
                  Array.from(
                    ...
                    b.querySelector("[class*='blookContainer'] > img") ||
                      a.push(
                        b.querySelector("[class*='packImgContainer'] > img")
                          .alt,
                      );
                    return a;
                  }, []),
              ...
              if (window.location.pathname.startsWith("/market")) {
                ...
                const prices = Array.prototype.reduce.call(
                  ...
                  (a, b) => {
                    b.querySelector("[class*='blookContainer'] > img") ||
                      (a[
                        b.querySelector("[class*='packImgContainer'] > img").alt
                      ] = parseInt(
                        b.querySelector("[class*='packBottom']").textContent,
                      ));
                    return a;
                  },
                  {},
          ...
          },
          {
            name: "Zoom Hack",
            description:
              "Removes the window is too tall so you can zoom out with arrow keys",
            type: "toggle",
            enabled: false,
            run: function () {
...
`;
          ...
          },
          {
            ...
            description:
              "Removes the Name Length. You can go up to 120 Characters",
            run: function () {
              (
                document.querySelector(
                  "body > main > div > form > div > div.Form_inputRow__w_TK3 > div > input",
                ) ||
                document.querySelector(
                  "#app > div > div > div:nth-child(2) > div > form > div._inputRow_1lycq_54 > div._nameInputContainer_1lycq_85 > input",
                )
          ...
          },
          {
            ...
            run: function (newname) {
              (async () => {
                ...
                const olddata =
                  await reactHandler().props.liveGameController.getDatabaseVal(
                    `c/${oldname}`,
                  );
                await reactHandler().props.liveGameController.removeVal(
                  `c/${oldname}`,
                );
            ...
            run: async function () {
              if (!window.location.href.includes("play.blooket.com"))
                ...
                const { t } = await fetch(
                  ...
                  .then((x) => x.json())
                  .catch(() =>
                    alert("There was an error creating a solo game."),
                  );
                await fetch(
                ...
                await fetch(
                  "https://play.blooket.com/api/gamequestionsets?gameId=" +
                    gameId,
                  {
                    credentials: "include",
                  },
                ...
                await fetch("https://play.blooket.com/api/users/factorystats", {
                  ...
                  alert("There was an error when spoofing stats."),
                );
          ...
          },
          {
            ...
            run: function () {
              function getGameCode() {
                ...
                if (appDiv) {
                  const reactComponent =
                    Object.values(appDiv)[1]?.children[0]?._owner;
                  return (
                    reactComponent?.stateNode?.props?.client?.hostId || null
                  );
                }
                return null;
              }
              ...
              const url = gameCode
                ? "https://blooketbot.schoolcheats.net/" + gameCode
                : "https://blooketbot.schoolcheats.net/";
              window.open(
                url,
                "_blank",
          ...
          },
          {
            ...
            run: function () {
              ...
              const gameCode = getGameCode();
              const url = gameCode
                ? "https://0alter0.github.io/blooketstreamfinderV2/" + gameCode
          ...
          },
          {
            ...
            run: function () {
              const lobby = window.location.pathname.startsWith("/play/lobby"),
                dashboard =
                  !lobby && window.location.pathname.startsWith("/blooks");
              if (dashboard) {
                let key = "konzpack",
                  ...
                  function (func) {
                    Object.prototype.hasOwnProperty.call = function () {
                      ...
                      return (Object.prototype.hasOwnProperty.call =
                        propCall).apply(this, arguments);
                    };
                    return func;
                  },
                ]);
                const blookData = webpack(4927).nK;
                const blooksHook = Object.values(
                  document.querySelector("[class*=BlooksWrapper_content]"),
                )[0].return.memoizedState.next;
                ...
                alert(
                  "This only works in lobbies or the dashboard blooks page.",
                );
            },
          },
          {
          ...
          },
          {
            ...
            run: async function () {
              if (!window.location.pathname.startsWith("/blooks")) {
                alert(
                  "This can only be run on the Blooks page. Please go to https://dashboard.blooket.com/blooks",
                );
              } else {
                if (
                  confirm(
                    "Are you sure you want to sell your duplicates? This will sell everything except Legendary and above, leaving you with 1 of each blook.",
                  )
                ) {
                  async function sellBlook(blookName, quantity) {
                    ...
                    formData.append(
                      "0",
                      '[{"status":"UNSET","message":"","fieldErrors":{}},"$K1"]',
                    );
                    try {
                      const response = await fetch(
                        "https://dashboard.blooket.com/blooks",
                        {
                          ...
                          headers: {
                            "next-action":
                              "504d5bfe3d4b01def79630a2138ec4f50f85bee9",
                      ...
                      if (response.ok) {
                        console.log(
                          `Successfully sold ${quantity}x ${blookName}`,
                        );
                      } else {
                        console.error(
                          `Failed to sell ${blookName}: ${response.status}`,
                        );
                      }
                    } catch (err) {
                  ...
                  window.blookslist = (() => {
                    ...
                    const fiberKey = Object.keys(anyEl).find((k) =>
                    ...
                    const path =
                      "child.child.child.child.child.child.child.child.child.child.sibling.child.child.child.child.child.child.child.child.sibling.child.child.sibling.child.child.child.child.sibling.child.sibling.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.sibling.child.sibling.sibling.sibling.sibling.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.child.sibling.child.child";
                    let target = fiber;
        ...
        cheats: [
          {
            name: "EXECUTOR",
            description:
              "Opens an advanced, sleek, GUI with countless features, made by @landsedge",
            run: function () {
              try {
                if (typeof window.ExecutorUserScriptVersion === "function") {
                  console.log(
                    "Executor Userscript detected — running the built-in version.",
                  );
          ...
          },
          {
            ...
            description:
              "Toggle between inverting and restoring colors on the page",
            run: function () {
              const html = document.documentElement;
              html.style.filter =
                html.style.filter === "invert(1)" ? "" : "invert(1)";
              ...
              for (let i = 0; i < elems.length; i++) {
                ...
                if (
                  ...
                  el.style.filter =
                    el.style.filter === "invert(1)" ? "" : "invert(1)";
                }
              }
            },
          },
            ...
            run: function () {
              var e = {
                ...
                mouseMove: function (t) {
                  ((e.orientation.yaw =
                    -(180 * Math.cos((Math.PI * t.clientX) / innerWidth)) *
                    e.limit.value),
                    (e.orientation.pitch =
                      180 *
                      Math.cos((Math.PI * t.clientY) / innerHeight) *
                      e.limit.value),
                    e.updateBody());
                },
                gyroMove: function (t) {
                  (innerWidth > innerHeight
                    ? ((e.orientation.yaw = -(t.alpha + t.beta)),
                      (e.orientation.pitch =
                        t.gamma - 90 * Math.sign(90 - Math.abs(t.beta))))
                    : ((e.orientation.yaw = -(t.alpha + t.gamma)),
                ...
                updateCSS: function () {
                  if (e.non.checked) e.cssDynamic.innerHTML = "";
                    ...
                    for (
                      var t = 0;
                      document.querySelector("body" + " > *".repeat(t));
                      t++
                    );
                    var a = e.gap.value / t,
                ...
                toggle: function () {
                  "active" == e.menu.className
                    ? e.menu.removeAttribute("class")
                    : (e.menu.className = "active");
                },
                quit: function () {
                ...
                init: function () {
                  ((document.body.parentNode.appendChild(e.menu).id =
                    "tri-menu"),
                    e.newRange(
                      e.limit,
                      "limit",
          ...
          },
          {
            ...
            run: function () {
              const html = document.documentElement;
              html.style.filter = html.style.filter.includes("blur")
                ? ""
                : "blur(3px)";
            },
          },
          ...
          },
          {
            ...
            run: function () {
              ...
              html.style.transform =
                html.style.transform === "scaleX(-1)" ? "" : "scaleX(-1)";
            },
          },
          {
    ...
    `;
          ...
          },
          {
            ...
            run: function () {
              const html = document.documentElement;
              html.style.transform =
                html.style.transform === "scale(1.1)" ? "" : "scale(1.1)";
            },
          },
          {
            name: "Spin Slowly",
          ...
          },
          {
            ...
            run: function () {
              const html = document.documentElement;
              html.style.transform =
                html.style.transform === "scaleY(-1)" ? "" : "scaleY(-1)";
            },
          },
          {
            name: "Shake UI",
    ...
    `;
          ...
          },
          {
            name: "Flip Random UI",
            description:
              "UI elements flip randomly THIS WILL BREAK GUI, YOU WILL NEED TO RELOAD!",
            run: function () {
              if (window._flipUI) {
    ...
    `;
          ...
          },
          {
            ...
            run: function () {
              ...
              window._rainbowText = setInterval(() => {
                document
                  .querySelectorAll("*")
                  .forEach((el) => (el.style.color = `hsl(${hue},90%,55%)`));
                hue += 3;
                if (hue > 360) hue = 0;
              }, 100);
            },
          },
          {
            ...
            run: function () {
              ...
              document.removeEventListener(
                "mousemove",
                window._cursorSparkleFn,
              );
              document.removeEventListener(
                "mousemove",
                window._cursorRainbowFn,
              );
          ...
          },
          {
            ...
            description:
              "Switch GUI font-size between normal and slightly smaller.",
            run: function () {
              if (typeof gui !== "undefined") {
                const current =
                  gui.style.fontSize || getComputedStyle(gui).fontSize;
      ...
      custom_modules: {
        img: (() => {
          const img = new Image();
          img.src =
            "https://media.blooket.com/image/upload/c_limit,f_auto,h_250,fl_lossy,q_auto:low/v1767747712/mxquoykv3onnjgmw6zgl.png";
          return img.src;
        })(),
        name: "Custom Modules",
        cheats: (() => {
          if (detectCSP()) return [];
          const customModules =
            CookieHelper.getCookie("custom_modules_list") || [];
          return customModules.map((module, index) => {
            const defaultDescription =
              module.description || `Custom Module #${index + 1}`;
        ...
        cheats: [
          ...
          },
          {
            ...
            run: function () {
              ...
              var da =
                reactHandler().stateNode.props.liveGameController._liveApp.database()
                  ._delegate._repoInternal.server_.onDataUpdate_;
              function handleChat(e, t) {
                if (t != null) {
                  if (e.includes("/msg")) {
                    t?.msg &&
                      (console.log(t.msg), a(e.split("/")[2] + ": " + t.msg));
                  }
                }
              }
          ...
          },
          {
            ...
            run: async () => {
              ...
              const players =
                await stateNode.props.liveGameController.getDatabaseVal("c");
              let freed = 0;
              ...
              async function blockUser(name) {
                ...
                const res = await fetch(
                  "https://fb.blooket.com/c/firebase/block",
                  {
                    headers: {
                      "Content-Type": "application/json",
                    },
            ...
            run: async () => {
              const stateNode = () =>
                Object.values(document.querySelector("#app>div>div"))[1]
                  .children[0]._owner.stateNode;
              (
                await stateNode().props.liveGameController.getDatabaseRef("")
              ).on("value", (e) => stateNode()?.getClients?.(!1));
            },
          },
          {
            name: "Anti-Flood",
                  ...
                  dbRef.on("value", (snapshot) => {
                    ...
                    for (const key in clients) {
                      if (!currentClients[key]) {
                        newClients.push(key);
                        clientCounts[key.replace(/[0-9]/g, "")] =
                          (clientCounts[key.replace(/[0-9]/g, "")] || 0) + 1;
                      }
                    }
                    currentClients = clients;
                    for (const client of newClients) {
                      if (
                        currentClients[client].g ||
                        clientCounts[client.replace(/[0-9]/g, "")] > 1
                      ) {
        ...
        cheats: [
          {
            ...
            run: function () {
              ...
              if (!this.enabled) {
                ...
                this.enabled = false;
                if (stateNode._choosePrize)
                  stateNode.choosePrize = stateNode._choosePrize;
              }
            },
          },
          {
                      ...
                      (players) => {
                        try {
                          ...
                          for (let i = 0; i < players.length; i++)
                            if (
                              players[i][0] != stateNode.props.client.name &&
                              players[i][1] > most
                            )
                              most = players[i][1];
                          for (
                            let i = 0;
                            i < stateNode.state.choices.length;
                            i++
                          ) {
                            ...
                            if (choice.type == "gold")
                              value =
                                stateNode.state.gold + choice.val ||
            ...
            run: function (userInput) {
              ...
              function setv(args) {
                getReactOwner().stateNode.props.liveGameController.setVal({
                  path:
                    "c/" +
                    getReactOwner().stateNode.props.client.name +
                    "/" +
                    args[0],
          ...
          },
          {
            ...
            run: function () {
              ...
              (document.body.append(e),
                (window.alert = e.contentWindow.alert.bind(window)),
                e.remove());
              let { props: t, state: a } = Object.values(
                  document.querySelector("body div[id] > div > div"),
                )[1].children[0]._owner.stateNode,
                o = 0;
      ...
      hack: {
        ...
        cheats: [
          ...
          },
          {
            ...
            run: function () {
              if (!this.enabled) {
                this.enabled = true;
                this.data = setInterval(
                  (state) => getStateNode().setState(state),
                  25,
                  {
                    choices: [
                      {
          ...
          },
          {
            ...
            description:
              "Freezes other players when they attempt to hack you *IF TURNED ON IT WILL LAG YOU LIKE HELL*",
            type: "toggle",
            enabled: !1,
            data: null,
            run: function () {
          ...
          },
          {
            ...
            description:
              "Shows password + crypto for every player in Crypto Hack",
            type: "button",
            run: function () {
        ...
        `;
                ...
                lgc.getDatabaseVal("c", (data) => {
                  if (!data) {
                    contentDiv.innerHTML =
                      "<div style='text-align:center;color:#aaa;'>No player data found.</div>";
                    return;
                  }
                  let html = "";
              ...
              loadPlayerData();
              document.getElementById("close-crypto-spy").onclick = () =>
                panel.remove();
              document.getElementById("refresh-crypto-spy").onclick =
                loadPlayerData;
            },
          },
          {
                ...
                options() {
                  ...
                  return new Promise((resolve) =>
                    e.props.liveGameController._liveApp
                      ? e.props.liveGameController.getDatabaseVal(
                          "c",
                          (data) => data && resolve(Object.keys(data)),
                        )
                      : resolve([]),
                  );
                },
            ...
            run: function () {
              ...
              function setv(args) {
                reactHandler().stateNode.props.liveGameController.setVal({
                  path:
                    "c/" +
                    reactHandler().stateNode.props.client.name +
                    "/" +
                    args[0],
                  val: args.slice(1, args.length).join(" "),
                });
                  ...
                  stateNode.props.liveGameController.getDatabaseVal(
                    ...
                    (players) => {
                      ...
                      if (
                        ...
                        if (steal <= 0) return;
                        const currentCrypto =
                          Number(stateNode.state.crypto) || 0;
          ...
          },
          {
            ...
            run: function (target, stealAmount) {
              if (!target || !Number.isFinite(stealAmount) || stealAmount <= 0)
                return;
              let stateNode = getStateNode();
              stateNode.props.liveGameController.getDatabaseVal(
                "c",
                    ...
                    (t().stateNode.setState({
                      ...
                      fish: {
                        ...
                        weight:
                          ((n = a[i].minWeight),
                          Math.floor(
                            Math.random() * ((s = a[i].maxWeight) - n) + n,
                          )),
                        tier: a[i].tiers[
                          Math.floor(Math.random() * a[i].tiers.length)
                        ],
                        isSpecial:
                          o.includes(i) && 8 > Math.floor(100 * Math.random()),
                      },
                      claimReady: !1,
                    }),
                      setTimeout(function () {
            ...
            run: function (lure) {
              (() => {
                ...
                function findAndDispatch(node) {
                  ...
                  if (
                    node.memoizedProps &&
                    typeof node.memoizedProps.lureLevel === "number"
                  ) {
                ...
                if (success) {
                  alert(
                    `✅ Lure Level permanently set to ${targetInternal + 1}!`,
                  );
                } else {
                  alert(
                    "❌ Could not find Lure State Hook. Try catching one fish first.",
                  );
                }
              })();
            },
          },
          {
            ...
            run: function (weight) {
              (() => {
                ...
                if (success) {
                  alert(
                    `✅ Weight set to ${NEW_WEIGHT.toLocaleString()}!\n\nCatch a fish to update the UI.`,
                  );
                } else {
            ...
            run: function () {
              if (!this.enabled) {
                ...
                this.data = setInterval(() => {
                  ...
                  if (this.imgs == null)
                    this.imgs = Array.prototype.map.call(
                      Array.prototype.slice.call(
                        document.querySelector("[class*=prizesList]").children,
                        1,
                        4,
                      ),
                      (x) => x.querySelector("img").src,
                    );
                  const esp = Object.values(
        ...
        cheats: [
          {
            ...
            run: function () {
              ...
              Object.keys(towersDict).forEach((typeKey) => {
                ...
                else if (group.children && group.children.entries)
                  towers = group.children.entries;
                else if (Array.isArray(group)) towers = group;
                towers.forEach((tower) => {
                  tower.cd = 0;
                  tower.fullCd = 1;
                  if (tower.stats) {
                    tower.stats.dmg = 1e10;
                    tower.stats.range = 1e10;
                    tower.stats.fireRate = 1e10;
                    tower.stats.ghostDetect = true;
                    tower.stats.maxTargets = 1e10;
              ...
              if (totalUpgraded > 0) {
                console.log(
                  `✅ Upgraded ${totalUpgraded} towers (DMG, Range, Ghost Hit)!`,
                );
              } else {
          ...
          },
          {
            ...
            run: function (health) {
              ...
              if (
                healthTarget.health === undefined &&
                phaser.scene.gameData?.health !== undefined
              ) {
        ...
        cheats: [
          ...
          },
          {
            ...
            run: function () {
              ...
              const monitor = setInterval(() => {
                const player =
                  phaser.player || phaser.scene.playerService?.player;
                if (player) {
...
`;
            ...
            run: (() => {
              ...
              function shortNum(value) {
                ...
                if (value >= 1000) {
                  ...
                  const suffixNum = Math.floor(
                    Math.floor(
                      (Math.log(value) / Math.log(10)).toPrecision(14),
                    ) / 3,
                  );
              ...
              return function () {
                if (!this.enabled) {
                  ...
                  this.data = setInterval(() => {
                    let stateNode = getStateNode();
                    const rocks = document.querySelector(
                      '[class*="rockButton"]',
                    ).parentElement.children;
                    if (
                      !Array.prototype.every.call(rocks, (element) =>
        ...
        cheats: [
          {
            ...
            run: function () {
              ...
              if (!node?.memoizedState)
                return console.log("❌ Failed to find game state.");
              const hook = getHook(node, 4);
              if (hook?.memoizedState?.Toast) {
                const newLevels = { ...hook.memoizedState };
            ...
            run: function (cafeCash) {
              ...
              while (hook) {
                if (
                  ...
                  return console.log(
                    `✅ Cash set to ${newAmount.toLocaleString()}!`,
                  );
                }
                hook = hook.next;
                depth++;
          ...
          },
          {
            name: "Stock Food",
            description:
              "Stocks all food to 999 (Not usable in shop) (Solo only)",
            run: function () {
              const node = getStateNode("cafe");
              if (!node?.memoizedState)
                return console.log("❌ Failed to find game state.");
              ...
              if (hook4?.memoizedState?.Toast && hook5?.memoizedState) {
                const newLevels = { ...hook4.memoizedState };
                const newStock = { ...hook5.memoizedState };
                Object.keys(newLevels).forEach((key) => {
                  if (typeof newLevels[key] === "number") newLevels[key] = 5;
                });
          ...
          },
          {
            ...
            run: function () {
              ...
              if (hook?.memoizedState?.toast?.prices) {
                const newDefs = { ...hook.memoizedState };
        ...
        cheats: [
          {
            ...
            run: function (blook) {
              ...
              const newBlook = { ...chosenBlook, level: 1, bonus: 1 };
              const maxSlots = 10;
          ...
          },
          {
            ...
            run: function () {
              ...
              function traverseDOM(node) {
                const k = Object.keys(node).find((key) =>
        ...
        cheats: [
          {
            ...
            run: function (numBlooks) {
              ...
              stateNode.props.liveGameController.setVal({
                path:
                  (stateNode.isTeam ? "a/" : "c/") +
                  stateNode.props.client.name +
                  "/bs",
                val: numBlooks,
              });
            },
                ...
                this.data = setInterval(
                  (stats) => {
                    ...
                    if (stateNode.state.phase == "choice") {
                      ...
                      Object.keys(stateNode.state.guest.yes || {}).forEach(
                        (x) => {
                          ...
                          element.innerText = String(
                            stateNode.state.guest.yes[x],
                          );
                          elements[x].appendChild(element);
                        },
                      );
                      Object.keys(stateNode.state.guest.no || {}).forEach(
                        (x) => {
                          if (elements[x] == null) return;
                          let element = document.createElement("div");
                          element.className = "choiceESP";
        ...
        cheats: [
          {
            ...
            run: function () {
              ...
              )[0].return.updateQueue.lastEffect.deps[0].current.config
          ...
          },
          {
            ...
            description:
              "Changes various game mechanics and lets you play with the spacebar",
            inputs: [
              {
                name: "Bird Gravity",
                type: "number",
            ...
            run: function (birdGravity, birdSpeed, birdFlapPower) {
              (function () {
                ...
                if (!canvas) return;
                const scene =
                  Object.values(canvas)[0].return.updateQueue.lastEffect.deps[0]
                    .current.config.sceneConfig;
                if (!scene) return;
            ...
            run: function (newHtml) {
              (function () {
                if (newHtml) {
                  ...
                  var scoreTextDiv = document.querySelector(
                    "div._scoreText_e2c5l_7",
                  );
                  if (scoreTextDiv) {
    ...
    searchbarButton.innerHTML =
      '<i class="fas fa-times" style="line-height: 1;"></i>';
    ...
    searchPage.onPath = searchbarHolder.onsubmit = (e) => {
      ...
      for (const child of searchResults.children) {
        if (child != noResult) {
          if (
            child.dataset[datasets.mode]?.includes?.(query) ||
            child.dataset[datasets.name].includes(query) ||
            child.dataset[datasets.description]?.includes?.(query)
          ) {
            hasResults = true;
            if (animEnabled) {
            ...
            if (child.dataset[datasets.mode]) {
              if (animEnabled) {
                gamemodeResults[child.dataset[datasets.mode]].style.animation =
                  "slideInLeft 0.3s ease-out";
              }
              gamemodeResults[child.dataset[datasets.mode]].style.display =
                "block";
            }
          } else {
            if (animEnabled) {
    ...
    favoritesPage.onPath = () => {
      noFavorites.style.display =
        favoritesPage.querySelector(
          "[data-" + datasets.favorited + "='true']",
        ) == null
          ? "block"
          : "none";
    };
    ...
    for (const mode in cheats) {
      ...
      searchResultSeparator.onclick = () =>
        path.push(cheats[mode].name, cheatsPage);
      searchResultSeparator.className = classes.searchResultSeparator;
      searchResultSeparator.dataset[datasets.name] =
        (searchResultSeparator.innerText = cheats[mode].name).toLowerCase();
      ...
      for (const cheat of cheats[mode].cheats) {
        ...
        searchResult.dataset[datasets.name] = (searchResultName.innerText =
          cheat.name +
          (cheat.type == "toggle" && !cheat.name.includes("toggle")
            ? " (Toggle)"
            : "")).toLowerCase();
        ...
        searchResult.dataset[datasets.description] =
          (searchResultDescription.innerText = cheat.description).toLowerCase();
        ...
        runButton.onclick = () => {
          try {
            cheat.run.apply(
              cheat,
              inputs.map((x) =>
                x.dataset[datasets.type] == "number"
                  ? parseFloat("0" + x.value)
                  : x.dataset[datasets.type] == "options"
                    ? JSON.parse(x.value)
                    : x.value,
              ),
            );
            if (cheat.type == "toggle") {
              ...
              Logs.addLog(
                `Toggled "${cheat.name}" ${cheat.enabled ? "on" : "off"}`,
                cheat.enabled ? "var(--toggleOn)" : "var(--toggleOff)",
              );
            ...
            Logs.addLog(
              `Error running "${cheat.name}": ${err.message}`,
              "var(--error)",
            );
            if (areSweetAlertsEnabled()) {
              Swal.fire({
      ...
      gamemode.onclick = () => {
          ...
          if (cspDetected) {
            ...
            try {
              if (cheatsPage && cheatsPage.style) {
                let blurTarget =
                  cheatsPage.querySelector &&
                  cheatsPage.querySelector(".xgui-blur-target");
                if (!blurTarget) {
            ...
            if (
              cheatsPage.querySelector &&
              cheatsPage.querySelector(".xgui-csp-notice")
            )
              return;
            ...
            try {
              if (
                !cheatsPage.style.position ||
                cheatsPage.style.position === "static"
              )
                cheatsPage.style.position = "relative";
            ...
            noticeText.innerHTML =
              "This is not supported on your version. Please use the extension version or get the CSP unblock extension.";
            ...
            extensionButton.href =
              "https://github.com/xgui-client/X-GUI-BLOOKET-CLIENT/raw/refs/heads/main/extension/x-gui-extension.zip";
        ...
        if (unsupportedModes.includes(mode)) {
          ...
          if (
            cheatsPage.querySelector &&
            cheatsPage.querySelector(".xgui-blur-notice")
          )
            return;
          const notice = document.createElement("div");
          ...
          const noticeText = document.createElement("div");
          noticeText.innerHTML =
            "Cheats for this gamemode are still in development. Join the discord to find out when its done.";
          noticeText.style.cssText = `
                        display: block !important;
                        color: #FFFFFF !important;
                        font-size: 15px !important;
...
.${classes.favoritesPage || "favoritesPage"}::-webkit-scrollbar-track {
  background: transparent;
    ...
    const versions = [
      [
        "Version 6.73x",
        "FIXED MAJOR BUG THAT BROKE ALL FIREBASE GAMEMODES AND ALOT OF OTHER FEATURES",
        "null",
      ],
      [
        "Version 6.72x",
        "ADDED V7 OPTOMIZATIONS TO FIX BUGS, ADDED ESP TO MONSTER BRAWL, ADDED NEW SECTION CALLED HOST, AND ADDED ZOOM HACK.",
        "null",
      ],
      ["Version 6.71x", "QUICK HOTFIX", "null"],
      [
        "Version 6.70x",
        "ADDED SMOOTH GUI ANIMATIONS, FIXED CUSTOM MODULE BUGS, ALLOWED TEXT VALUES FOR STATS, FIXED CRYPTO STEALING, ADDED SPAM CRYPTO HACK, UPDATED ICONS, FIXED STORAGE SYNC, AND UPDATED CREDITS.",
        "null",
      ],
      [
        "Version 6.69x",
        "ADDED CSP DETECTOR FOR CUSTOM MODULES, FIXED CUSTOM MODULES BUG, FIXED STORAGE ISSUE ADDED IMAGE FIX AND ADDED FREE PLUS!.",
        "null",
      ],
      [
        "Version 6.68x",
        "ADDED SMOOTH ANIMATIONS FOR X-GUI, ADDED CUSTOM MODULES, AND NOW YOU CAN DRAG THE TOP AREA.",
        "null",
      ],
      [
        "Version 6.67x",
        "ADDED SWAL ALERTS! TOGGLE, ADDED UNSUPPORTED GAMEMODE WARNINGS, AND MOVED SETTINGS MENU.",
        "null",
      ],
      ...
      [
        "Version 6.60x",
        "fixed ping display! ADDED CONSOLE MESSAGE AND ADDED PIN GUESSER!!",
        "null",
      ],
      [
        "Version 6.50x",
        "FIXED KEYBINDS!! ADDED CHEATS AND ADDED 8 NEW THEMES, HAPPY HOLIDAYS GUYSS <3",
        "null",
      ],
      ...
      [
        "Version 6.20x",
        "ADDED MORE CHEATS+++",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 6.10x",
        "ADDED WHAT YOU ARE READING RN",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 6.00x",
        "ADDED MOBILE MODE + A BUNCH MORE CRAP",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 5.90x",
      ...
      [
        "Version 5.20x",
        "ADDED REMOVE ALL TAKEN BLOOKS TO THE GLOBAL STATENODE",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 5.10x",
      ...
      [
        "Version 5.00x",
        "ADDED FREEZE LEADERBOARD",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 4.90x",
        "ADDED CHANGE GAME CODE TO FLAPPY BLOOK",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 4.80x",
        "ADDED CLIENT SIDE FRENZY",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 4.70x",
      ...
      [
        "Version 4.40x",
        "FIXED THE DOUBLE ERROR (SORRY GUYS!)",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 4.30x",
      ...
      [
        "Version 3.70x",
        "ATTEMPTED AN UPDATE BUT FAILED :(",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 3.60x",
      ...
      [
        "Version 3.20x",
        "ADDED SIMULATE UNLOCK, ALWAYS HACK, HOST ANY GAMEMODE, BYPASS FILTER, CRASH HOST, STEAL ALL PLAYERS GOLD AND BYPASS JOIN LIMIT",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 3.10x",
      ...
      [
        "Version 2.90x",
        "ADDED BYPASS NAME FILTER",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 2.80x",
        "ADDED 3 NEW CHEATS TO CRYPTO AND GOLD QUEST",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 2.70x",
      ...
      [
        "Version 2.60x",
        "ADDED GLOW EFFECT TO TITLE",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 2.50x",
        "REVAMPED COLORS AND UI",
        "https://greasyfork.org/en/scripts/553301-x-gui-client-for-blooket/versions",
      ],
      [
        "Version 2.40x",
...
`;
    ...
    addAltCard.querySelector("#toggleAddPass").onclick = () => {
      addPassInput.type =
        addPassInput.type === "password" ? "text" : "password";
    };
    ...
    function minifyJS(code) {
      try {
        return code
          ...
          .replace(/\s*([{}();,:<>!=+\-*/%&|?])\s*/g, "$1")
          .replace(
            /\b(return|var|let|const|if|else|for|while|function|class)\s+/g,
            "$1 ",
          )
          .trim();
      } catch (e) {
...
`;
    ...
    const updateButtonGrid = () => {
      const moduleCount = (CookieHelper.getCookie("custom_modules_list") || [])
        .length;
...
`;
    createBtn.onclick = async () => {
      const { value: formValues, isDismissed } = await Sweetalert2.fire({
        title:
          '<span>Create New Module</span><button class="helpBtn" id="help-btn" type="button">?</button>',
        html:
...
function(amount) {
    ...
    function showModuleManager() {
      ...
      modules.forEach((module, index) => {
        const escapedName = module.name
          .replace(/</g, "&lt;")
          .replace(/>/g, "&gt;");
        const escapedDesc = (module.description || "No description")
          .replace(/</g, "&lt;")
          .replace(/>/g, "&gt;");
        moduleListHTML += `
    ...
    async function editModule(index) {
      ...
      const { value: formValues, isDismissed } = await Sweetalert2.fire({
        title:
          '<span>Edit Module</span><button class="helpBtn" id="help-btn-edit" type="button">?</button>',
        html:
    ...
    codingCredits.append(createCredit("Module Dev", "redhorse26+landsedge"));
    codingCredits.append(
      createCredit("Design+Module Dev", "Lil Skittle+landsedge"),
    );
    codingCredits.append(createCredit("Contributor", "DannyDan"));
    codingCredits.append(
      createCredit(
        "Original Blooket Cheats",
        'gliz <i class="fas fa-long-arrow-alt-right"></i> Minesraft2 <i class="fas fa-long-arrow-alt-right"></i> 05Konz',
      ),
    );
    ...
    creditLinks.append(
      createCredit(
        "Our Github",
        '<a target="_blank" href="https://github.com/XULL-XXX/X-GUI-BLOOKET-CLIENT">X-GUI CHEATS/GITHUB</a>',
      ),
    );
    creditLinks.append(
      createCredit(
        "Our Website",
        '<a target="_blank" href="https://xgui-client.github.io">More Info/Website</a>',
      ),
    );
    creditLinks.append(
      createCredit(
        "Discord",
        '<a target="_blank" href="https://discord.gg/A7CRUz8vFR">JOIN THE SERVER</a>',
      ),
    );
    ...
    try {
      ...
      if (latestProcess != -1)
        uploadDates.append(createCredit("", parseDate(null)));
...
`;
    ...
    function updatePing() {
      ...
      img.src =
        "https://ac.blooket.com/dashclassic/favicon.ico?cb=" + Date.now();
    }
    updatePing();
    ...
    function addSetting(name, description, input, onUpdate) {
      ...
      searchResultDescription.className = classes.searchResultDescription;
      searchResult.dataset[datasets.description] =
        (searchResultDescription.innerText = description).toLowerCase();
      searchResultInfo.append(searchResultName, searchResultDescription);
      searchResult.onclick = () => {
      ...
      if (input.type == "keybind") {
        ...
        inputField.onclick = async () => {
          ...
          locked = true;
          inputField.data = await input.listen(
            (e) => (inputField.value = e + "..."),
          );
          locked = false;
      ...
      if (input.type == "toggle") {
        ...
        runButton.onclick = () =>
          onUpdate(
            inputField.dataset[datasets.type] == "number"
              ? parseFloat("0" + inputField.value)
              : inputField.dataset[datasets.type] == "options"
                ? JSON.parse(inputField.value)
                : (inputField.data ?? inputField.value),
          );
      }
    ...
    addSetting(
      ...
      "Shortcut to hide to GUI",
      {
        ...
        listen: (change) =>
          createKeybindListener((keys) => change(parseKeybind(keys))),
      },
      (x) => {
    ...
    addSetting(
      "Highlight 1",
      "Hover color, sub-text color, button color, and input outlines",
      {
        type: "string",
        name: "CSS Value",
      ...
      (x) =>
        gui.style.setProperty(
          "--highlight",
          Settings.setItem("theme.highlight", x || variables["--highlight"]),
        ),
    );
    addSetting(
      "Highlight 2",
      "Credits page's warning message color",
      {
        type: "string",
        name: "CSS Value",
      ...
      (x) =>
        gui.style.setProperty(
          "--highlight2",
          Settings.setItem("theme.highlight2", x || variables["--highlight2"]),
        ),
    );
    addSetting(
      "Background",
      ...
      (x) =>
        gui.style.setProperty(
          "--background",
          Settings.setItem("theme.background", x || variables["--background"]),
        ),
    );
    addSetting(
      "Background 2",
      "Secondary GUI background color",
      {
        type: "string",
        name: "CSS Value",
    ...
    addSetting(
      ...
      (x) =>
        gui.style.setProperty(
          "--textColor",
          Settings.setItem("theme.textColor", x || variables["--textColor"]),
        ),
    );
    addSetting(
      "Text Color 2",
      "Credit page's contributor color",
      {
        type: "string",
        name: "CSS Value",
      ...
      (x) =>
        gui.style.setProperty(
          "--textColor2",
          Settings.setItem("theme.textColor2", x || variables["--textColor2"]),
        ),
    );
    addSetting(
      "Toggle (On)",
      "Enabled toggle button color",
      {
        type: "string",
        name: "CSS Value",
      ...
      (x) =>
        gui.style.setProperty(
          "--toggleOn",
          Settings.setItem("theme.toggleOn", x || variables["--toggleOn"]),
        ),
    );
    addSetting(
      "Toggle (Off)",
      "Disabled toggle button color",
      {
        type: "string",
        name: "CSS Value",
      ...
      (x) =>
        gui.style.setProperty(
          "--toggleOff",
          Settings.setItem("theme.toggleOff", x || variables["--toggleOff"]),
        ),
    );
...
`,
    ...
    (leaderboardPath = createSidebarPath(
      "Leaderboard",
      "trophy",
      leaderboardPage,
    )).style.display = "none";
...
`;
    ...
    function createCSPOverlay(targetPage) {
      if (
        ...
        targetPage.querySelector(".xgui-csp-notice")
      )
        return;
      try {
        if (targetPage && targetPage.style) {
          let blurTarget =
            targetPage.querySelector &&
            targetPage.querySelector(".xgui-blur-target");
          if (!blurTarget) {
            blurTarget = document.createElement("div");
            blurTarget.className = "xgui-blur-target";
    ...
    if (gui.querySelector("i").clientHeight == 0) {
      ...
      link.rel = "stylesheet";
      link.href =
        "https://ka-f.fontawesome.com/releases/v6.5.1/css/pro.min.css";
      gui.prepend(link);
    }
        ...
        function scanDeep(obj, depth = 0) {
          ...
          if (!factory && obj.factories && Array.isArray(obj.factories)) {
            if (obj.factories.length > 0) {
              ...
              if (
                sample.price &&
                Array.isArray(sample.price) &&
                sample.level !== undefined
              ) {
                factory = obj;
              }
            } else if (
              typeof obj.addCash === "function" &&
              obj.cash !== undefined
            ) {
              factory = obj;
            }
    ...
    function GetPhaser() {
      ...
      if (!window._phaserSceneHooked) {
        ...
        SceneManager.update = function (time, delta) {
          ...
          window._SCENE = this.scenes.find(
            (s) => s.sys?.isActive() && s.sys?.settings?.key !== "Boot",
          );
        };
        window._phaserSceneHooked = true;
      }
      if (
        !window._phaserGroupHooked &&
        window.Phaser.GameObjects?.Group?.prototype
      ) {
      ...
      if (
        !window._phaserSpriteHooked &&
        window.Phaser.GameObjects?.Sprite?.prototype
      ) {
      ...
      if (
        !window._phaserBodyHooked &&
        window.Phaser.Physics?.Arcade?.Body?.prototype
      ) {
      ...
      if (
        !window._phaserWorldHooked &&
        window.Phaser.Physics?.Arcade?.World?.prototype
      ) {
        const World = window.Phaser.Physics.Arcade.World.prototype;
        const checkInvincibility = (body1, body2) =>
          window._invincibilityActive &&
          (body1 === window._playerBody || body2 === window._playerBody);
        if (World.collideObjects) {
          const originalCollide = World.collideObjects;
          World.collideObjects = function (body1, body2, ...args) {
            return checkInvincibility(body1, body2)
              ? false
              : originalCollide.apply(this, [body1, body2, ...args]);
          };
        }
        if (World.separate) {
          ...
          World.separate = function (body1, body2, ...args) {
            return checkInvincibility(body1, body2)
              ? false
              : originalSeparate.apply(this, [body1, body2, ...args]);
          };
        }
      ...
      if (!window._SCENE) {
        const scenes =
          window.Phaser.Scene.SceneManager?.prototype?.scenes || [];
        window._SCENE = scenes.find(
          (s) => s?.sys?.isActive?.() && s.sys?.settings?.key !== "Boot",
        );
      }
    ...
    const getSystem = (() => {
      ...
      return () => {
        ...
        if (
          h === "play.blooket.com" ||
          h === "dashboard.blooket.com" ||
          !h.endsWith(".blooket.com")
        )
          return "menu";
